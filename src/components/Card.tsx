...
const Card: React.FC<CardProps> = ({ expression, meaning, reading }) => {
  return (
    <div className="relative group w-64 h-48 bg-white rounded-md shadow-md transition-transform duration-500 hover:cursor-pointer">
      ...
      <div className="absolute w-full h-full flex items-center justify-center text-3xl text-gray-800 bg-gray-200 rounded-md transition-opacity duration-300 opacity-0 group-hover:opacity-100">
        {meaning}
      </div>
    </div>
  );
